...
class KNN:
    ...
    def compute_distances_two_loops(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                for i_train in range(num_train):
                    # TODO: Fill dists[i_test][i_train]
                    dists[i_test][i_train] = math.sqrt(sum((self.train_X[i_train] - X[i_test])**2))
        return dists
    ...
    def compute_distances_one_loop(self, X):
        ...
        if self.metric == 'manhattan':
            ...
            for i_test in range(num_test):
                # TODO: Fill the whole row of dists[i_test]
                # without additional loops or list comprehensions
                dists[i_test] = math.sqrt(np.sum(((self.train_X - X[i_test])**2)), axis=1)
        return dists
    ...
    def compute_distances_no_loops(self, X):
        ...
        if self.metric == 'euclidean':
            # TODO: Implement computing all distances with no loops!
            dists = np.sqrt(np.sum(self.train_X**2, axis=1) - 2 * X.dot(self.train_X.T) + np.sum(X**2, axis=1)[:, np.newaxis])
        return dists
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            close = []
            a = np.argsort(dists[i, :])[:self.k]
            close = self.train_y[a]
            pred[i] = np.argmax(np.bincount(close))
        return pred
